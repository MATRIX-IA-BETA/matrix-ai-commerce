...
async function fetchPackConversation(packId, account) {
  ...
  if (!response.ok) throw new Error(`Mercado Livre recusou a leitura da conversa: ${JSON.stringify(data)}`);
...
async function generateSuggestion({ order, messages, products, sellerId }) {
  ...
  const history = messages.slice(-16).map(m => {
    const who = cleanId(m?.from?.user_id) === sellerId ? "SHOP MATRIX" : "CLIENTE";
    const text = textOf(m).slice(0, 900);
    return text ? `${who}: ${text}` : null;
  }).filter(Boolean).join("\n");

  const lastInbound = [...messages].reverse().find(m => cleanId(m?.from?.user_id) !== sellerId && textOf(m));
...
`;
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({ model: OPENAI_MODEL, instructions: BASE_PROMPT, input })
  });
  const data = await response.json().catch(() => ({}));
...
async function loadContext(packId) {
  ...
  if (!orderId) throw new Error("Não consegui identificar o pedido desta conversa.");
  const order = await findOrder(orderId, account);
  if (!order) throw new Error("Não consegui carregar o pedido do cliente.");
  const products = await buildProductContext(order, messages, account);
  return { safePack, account, messages, orderId, order, products };
}
...
async function learnFromEdit({ packId, suggestion, finalText }) {
  ...
  const ctx = await loadContext(packId);
  const sellerId = cleanId(ctx.account.user_id);
  const lastInbound = [...ctx.messages].reverse().find(m => cleanId(m?.from?.user_id) !== sellerId && textOf(m));
  const productNames = ctx.products.map(p => `${p.title} (${p.item_id}${p.seller_sku ? ` / SKU ${p.seller_sku}` : ""})`).join("; ");
