...
router.get('/sales/center',async(req,res)=>{try{
 ...
 const ids=(fin||[]).map(x=>String(x.marketplace_order_id)).filter(Boolean);let orders=[];
 ...
 const map=new Map(orders.map(o=>[String(o.marketplace_order_id),o]));
 const rows=[];
 for(const s of manual||[])rows.push({id:s.id,key:`manual:${s.id}`,origin:'Venda manual',sale_number:s.sale_number,date:s.sale_date,customer:s.customer_name||'Consumidor não identificado',product:null,gross_amount:s.gross_amount,net_amount:s.net_sale_amount,cost:s.total_cost,profit:s.total_profit,margin:s.margin_percent,stock_status:s.stock_status,status:s.status,nfe_amount:s.nfe_amount,nfe_status:s.nfe_status,nfe_number:s.nfe_number});
 for(const f of fin||[]){const o=map.get(String(f.marketplace_order_id))||{},items=o.raw_data?.order_items||[],product=items.map(x=>x?.item?.title||x?.title).filter(Boolean).join(' + ')||null;rows.push({id:String(f.marketplace_order_id),key:`ml:${f.marketplace_order_id}`,origin:'Mercado Livre',sale_number:String(f.marketplace_order_id),date:o.date_created||f.updated_at,customer:o.buyer_nickname||'Cliente Mercado Livre',product,gross_amount:o.paid_amount??o.total_amount??null,net_amount:f.actual_net_received,cost:f.total_cost,profit:f.total_profit,margin:f.margin_percent,stock_status:f.stock_status,status:'active',nfe_amount:f.nfe_amount,nfe_status:null,nfe_number:f.nfe_number});}
