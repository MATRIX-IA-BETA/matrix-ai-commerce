...
router.get('/sales/center',async(req,res)=>{try{
 ...
 for(const f of visibleFin){const o=map.get(String(f.marketplace_order_id))||{},items=o.raw_data?.order_items||[],product=items.map(x=>x?.item?.title||x?.title).filter(Boolean).join(' + ')||null,meta=f.metadata||{};rows.push({id:String(f.marketplace_order_id),key:`ml:${f.marketplace_order_id}`,origin:'Mercado Livre',sale_number:String(f.marketplace_order_id),date:o.date_created||f.updated_at,customer:meta.local_customer_name||o.buyer_nickname||'Cliente Mercado Livre',product,gross_amount:o.paid_amount??o.total_amount??null,net_amount:f.actual_net_received,cost:f.total_cost,profit:f.total_profit,margin:roi(f.total_profit,f.total_cost),stock_status:f.stock_status,status:meta.voided?'voided':'active',nfe_amount:f.nfe_amount,nfe_status:null,nfe_number:f.nfe_number});}
