...
const profitMargin=(profit,cost)=>Number(cost)>0&&profit!=null?Number(((Number(profit)/Number(cost))*100).toFixed(2)):null;
...
router.get('/sales/period-summary',async(req,res)=>{try{const rows=await periodRows(req.query.date_from,req.query.date_to,String(req.query.origin||''));const active=rows.filter(r=>!['cancelled','voided'].includes(String(r.status||'').toLowerCase())),known=active.filter(r=>r.profit!=null&&r.cost!=null),totalSales=active.reduce((s,r)=>s+Number(r.gross_amount||0),0),totalReceived=active.reduce((s,r)=>s+Number(r.net_amount||0),0),totalCost=active.reduce((s,r)=>s+Number(r.cost||0),0),knownCost=known.reduce((s,r)=>s+Number(r.cost||0),0),totalProfit=known.reduce((s,r)=>s+Number(r.profit||0),0);res.json({sucesso:true,total:active.length,total_vendas:money(totalSales),total_liquido:money(totalReceived),total_custo:money(totalCost),total_lucro:money(totalProfit),margem_lucro_percent:profitMargin(totalProfit,knownCost),lucros_conhecidos:known.length,custos_conhecidos:active.filter(r=>r.cost!=null).length,custo_base_margem:money(knownCost)})}catch(e){res.status(500).json({sucesso:false,mensagem:e.message})}});
